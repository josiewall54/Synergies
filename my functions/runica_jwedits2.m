...
function [weights,sphere,meanvar,bias,signs,lrates,data,y] = runica_jwedits2(data, varargin) % NB: Now optionally returns activations as variable 'data' -sm 7/05
...
if strcmpi(interupt, 'on')
  ...
  if nargout > 6 | strcmp(posactflag,'on')
      ...
      if strcmp(pcaflag,'off')
         ...
         %sub in weights here, but need to divide by eigenvecs and sphere
         %   of current data
         %insert this as a parameter, to be able to evaluate against test
         %     data (5/20/19 JJW)
          fixedW = [...          
              -13.3578   -1.1931  -13.1028   -3.8458   -1.2036   -6.9869  -10.0098   -7.1809   -8.8900   -1.4330;
               -9.9549    0.8857    9.9003   49.7466   -8.4675   13.1094    8.0174   -3.3441   -9.7300   -0.3778;
               -0.3886   -2.2819    0.5612    1.1684    0.0048    3.0350  -28.3105  -25.0292    3.0753   -3.7354;
              -22.3365    1.5723   31.6456   29.2898   -3.0008    1.2330    5.3681   -1.3285   -7.8235    0.6373];
        
          weights = fixedW / eigenvectors(:,1:ncomps)' / sphere;
