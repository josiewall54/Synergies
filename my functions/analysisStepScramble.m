function [] = analysisStepScramble(file, saveDir)
    ...
    %this version of getStepsConcat scrambles the steps
    ...
    %plotSubspacesFA(whiteStepsConcat, whiteStepResidualsConcat, 1:6, labels, saveDir)
...
function [stepsConcat, steps] = getStepsConcat(filteredEmg, GOOD_MUSCLES, gait, NORM_STEP_BIN_NUM)
    ...
    for j = 1:size(steps,2)
        ...
        %need new scrambled indices for each muscle
        scrambleIdx = shuffle(1:size(steps,3));
        
        for i = 1:size(steps,3)
            startIdx = (i-1) * 100 + 1;
            stopIdx = i * 100;
            stepsConcat(j,startIdx : stopIdx) = steps(:,j,scrambleIdx(i)).';
        end
