function [] = analysisDecorrelated(file, saveDir)
    ...
    %additional data massaging to decorrelate residuals
    scrambledResidualsConcat = scrambleResiduals(steps);
    decorrelatedStepsConcat = stepsConcat - stepResidualsConcat + scrambledResidualsConcat;
    decorrelatedSteps = reshape(decorrelatedStepsConcat, [size(decorrelatedStepsConcat, 1) 100 size(decorrelatedStepsConcat, 2) / 100]);
    decorrelatedSteps = permute(decorrelatedSteps, [2 1 3]);
    decorrelatedStepResidualsConcat = getStepResidualsConcat(decorrelatedStepsConcat, decorrelatedSteps);
    ...
    [icapcaStruct.avgR, icapcaStruct.avgRFolds, icapcaStruct.rAll, icapcaStruct.builtData] = ...
        computeEvalWrapper(decorrelatedStepsConcat, decorrelatedStepResidualsConcat, BUILD_PROP, 'icapca');   
    %plotSynNumVsCorr(icapcaStruct, saveDir)
    
    
   %plotSubspaces(stepsConcat, stepResidualsConcat, 2:6, labels, saveDir)

end

function scrambledResidualsConcat = scrambleResiduals(steps)
    stepLength = size(steps, 1);
    stepCount = size(steps, 3);
    muscleCount = size(steps, 2);
    
    avgStep = mean(steps, 3);
    res = steps - avgStep;
    
    scrambledResiduals = zeros(size(res));
    for i = 1:muscleCount
        thisRes = squeeze(res(:,i,:));
        scrambled = zeros(size(thisRes));
        for j = 1:stepLength
            scrambled(j,:) = thisRes(j, randperm(size(thisRes, 2)));
        end
        scrambledResiduals(:,i,:) = scrambled;
    end
    
    scrambledResidualsConcat = reshape(scrambledResiduals, [muscleCount, stepLength * stepCount]);
    
    
    
%     scrambledResidualsConcat = zeros(muscleCount, stepLength * stepCount);
%     newAssign = (stepCount - 1).*rand(size(scrambledResidualsConcat)) + 1;
%     
%     for i = 1:muscleCount
%         for j = 1:stepCount * stepLength
%             %currentStep = ceil(j / stepLength);
%             intraStepIdx = mod(j - 1, stepLength) + 1;
%             scrambledResidualsConcat(i,j) = res(intraStepIdx, i, round(newAssign(i,j)));
%         end
%     end   
end
